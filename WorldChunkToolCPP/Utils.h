...
namespace Utils
{
	...
	const static std::vector<std::vector<uint8_t>> chunkKeys = std::vector<std::vector<uint8_t>>
	{
	...
	static void DecryptChunk(std::vector<uint8_t>& data, const int dictionaryCount)
	{
		int keyPos = chunkKeyPattern[dictionaryCount + 8];
		std::vector<uint8_t> chunkKey = chunkKeys[keyPos]; // will it be optimized to take the reference
