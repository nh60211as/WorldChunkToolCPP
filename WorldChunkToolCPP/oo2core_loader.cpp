...
oo2core_loader::oo2core_loader(const std::string& oo2coreFilePath) noexcept :
...
oo2core_loader::~oo2core_loader() noexcept
{
    if (libraryInstance != nullptr)
        FreeLibrary(libraryInstance);
}

int oo2core_loader::Decompress(uint8_t* buffer, int size, uint8_t* outputBuffer, int uncompressedSize) const noexcept
{
...
bool oo2core_loader::is_oo2core_8_win64_legit() const noexcept
{
    return true;
}
